...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $email = $_SESSION['employer_data']['email']; // Assuming the email is stored in '$_SESSION['employer_data']['email']'

    // Fetch the current password hash from the database
    $result = mysqli_query($conn, "SELECT password FROM activated_employer WHERE email = '$email'");
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $update_sql = "UPDATE activated_employer SET password = '$hashedPassword' WHERE email = '$email'";
        ...
        echo json_encode(array("success" => false, "message" => "Employer not found"));
        exit();
    }
}
?>
