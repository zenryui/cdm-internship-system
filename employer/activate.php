...
if (isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        ...
        $sql = "SELECT * FROM pending_employer";
        ...
        while($row = mysqli_fetch_assoc($result)) {
            if (password_verify($code, $row['code'])) {
                ...
                $insert_sql = "INSERT INTO activated_employer (name, email, password, code, active) VALUES ('{$user_data['name']}', '{$user_data['email']}', '{$user_data['password']}', '{$user_data['code']}', 1)";
                if (mysqli_query($conn, $insert_sql)) {
                    // Delete user data from 'users' table
                    $delete_sql = "DELETE FROM pending_employer WHERE code = '{$user_data['code']}'";
...
<script src="../js/jax-script.js"></script>
